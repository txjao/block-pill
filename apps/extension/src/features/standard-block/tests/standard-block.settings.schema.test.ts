...
describe('standard block settings schema', () => {
  it('accepts the cooldown boundaries', () => {
    expect(
      standardBlockSettingsSchema.parse({ globalCooldownMilliseconds: MINIMUM_COOLDOWN_MS }),
    ).toEqual({ globalCooldownMilliseconds: MINIMUM_COOLDOWN_MS });
    expect(
      standardBlockSettingsSchema.parse({ globalCooldownMilliseconds: MAXIMUM_COOLDOWN_MS }),
